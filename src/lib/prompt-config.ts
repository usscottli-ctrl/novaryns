...
// 「AI帮写 / 智能优化」按功能页附加的分工具写法指令。
// key = 前端传来的 tool 标识;路由会以【优先级最高】前缀拼在通用 system 之后,
// 与通用要求冲突时以本条为准(否则通用的"写整图成品大片"会盖过功能页语义)。
// 没命中的工具(如 generate)不附加,按通用生图写法。
export const ASSIST_TOOL_HINTS: Record<string, string> = {
  suite:
    "本次是「一键电商套图」页:用户要为产品出整套营销图。你要写的是**这个产品的卖点与营销调性描述**(核心卖点、目标人群、使用场景、希望的画面风格),供套图规划使用;**不要**写单张图的完整生图提示词(不写构图/机位/光线细节)。",
  variations:
    "本次是「图裂变」页:基于上传图片做创意变体。你要写的是**变体方向**(改什么、往哪个主题/季节/配色/风格变),1-2 句;**绝不要**从零描述整张新图,也不要复述原图内容或写构图/机位/光线。",
  fusion:
    "本次是「AI 融图」页:多张素材合成一张图。你要写的是**合成关系**(谁是主体、谁作背景/点缀、空间摆放、光影如何统一),强调无缝自然,1-2 句;**不要**写单品成品大片式的完整生图提示词。",
  inpaint:
    "本次是「局部改图」页:只改用户涂抹的区域。你要写的是**针对涂抹区域的改动指令**(把什么改成什么),1-2 句、明确直白;**绝不要**描述整张画面、场景、构图或光线。",
  extract:
    "本次是「印花提取」页:用户要把图中的印花/图案提取成独立平面素材。你要写的是**提取指引**:看图指出要提取的印花在哪个部位、图案内容与配色,以及对提取结果的要求(忠于原图案与配色、边缘干净、平铺展开、透明底),1-2 句;**绝不要**写模特、穿搭、场景、构图、光线之类的整图生图提示词。",
  tryon:
    "本次是「虚拟试穿」页:模特试穿上传的服装。你要写的是**穿着与人物氛围要求**(保留服装哪些细节、模特姿态、气质、整体色调),1-2 句;服装本身的款式/颜色/材质不许改;**不要**写与试穿无关的场景大片描述。",
  titles:
    "本次是「爆款标题」页的产品卖点输入:提炼该产品的**核心卖点、目标人群与使用场景关键词**,输出逗号分隔的中文短语列表;**不是**完整句子、更不是生图提示词。",
};
